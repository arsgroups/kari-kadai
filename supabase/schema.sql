...
-- Restocks the returned quantity. If the returned product is a yield-child,
-- restocks its PARENT instead (converted through the yield/unit ratio),
-- exactly mirroring how the original sale deducted it -- children never
-- hold their own tracked stock (see trg_sale_item_stock_movement /
-- yield_parent_of further below). The parent invoice's subtotal/gst_amount
...
-- ============================================================================
-- INVOICE BRANDING — configurable header/footer images (Settings page),
-- stored in a public Storage bucket. Falls back to the bundled default
-- header and no footer image when unset.
-- ============================================================================
...
-- ============================================================================
-- ADMIN RESTORE HELPERS -- used only by the local backup/restore tool in
-- scripts/backup/. A plain re-INSERT of backed-up rows via PostgREST would
-- fire every trigger (stock deductions, average-cost bumps, audit log
-- entries...) exactly like a real new transaction, double-counting all of
-- it against data that already reflects those effects -- restore disables
-- triggers first, inserts the raw rows, then re-enables them. Both raise
